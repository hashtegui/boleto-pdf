...
// Gerar PDF do boleto Bradesco
bradescoPDF(boleto)
  .then(pdf => {
    fs.writeFile('boleto-bradesco.pdf', pdf, 'binary', err => {
      if (err) {
        console.log('Erro ao salvar PDF:', err)
        return
      }
      console.log('Boleto Bradesco gerado com sucesso: boleto-bradesco.pdf')
    })
  })
  .catch(err => {
    console.log('Erro ao gerar boleto Bradesco:', err)
  })
