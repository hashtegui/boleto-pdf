...
const safraPDF = require('../../dist/index').safra
...
const boleto = {
  ...
  recibo: {
    ...
    agency: '0123',
    agencyDigit: '4',
    expirationDay: new Date(2024, 11, 31),
    documentDate: new Date(2024, 11, 1),
    processingDate: new Date(2024, 11, 1),
    card: '101',
    documentNumber: '98765-4',
    formatedOurNumber: '000012345678-9',
    formatedValue: 'R$ 1.250,00',
...
// Gerar PDF do boleto Safra
safraPDF(boleto)
  .then(pdf => {
    const outputPath = path.join(__dirname, 'boleto-safra.pdf')
    fs.writeFile(outputPath, pdf, 'binary', err => {
      if (err) {
        console.log('Erro ao salvar PDF:', err)
        return
      }
      console.log('Boleto Safra gerado com sucesso: ' + outputPath)
    })
  })
  .catch(err => {
    console.log('Erro ao gerar boleto Safra:', err)
  })
